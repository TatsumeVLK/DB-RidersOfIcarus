...
const itemDefault = "AC5_0_6000;¸¶½Å Á©³ª¸®½ºÀÇ ¸ñ°ÉÀÌ;0.000000;60.000000;0.000000;135.000000;GD_WR_TF_AC_WZ_PR_DO_MG;*;*;7.000000;ne;30.000000;130547.000000;13055.000000;0.000000;0.000000;1808.000000;(PK°ø°Ý·Â%,100);(ÄðÅ¸ÀÓ%,-13);(¸ðµç°ø°Ý·Â%,5);*;1.000000;4.000000;4.000000;2500,2500,2500,2500;*;0.000000;0.000000;5.000000;r1;0.000000;0.000000;0.000000;0.000000;req;equip;ch;3.000000;*;0.000000;Auction_301;*;0.000000;0.000000;GU_Test_3_0435;AC0_EV_4_0_0433;Legend_0060_ne;ac_7_60;1.000000;0.000000;0.000000;0.000000;0.000000;0.000000;7.000000;CO_OR_A60_0001;CO_OR_A60_0002;*;60.000000;0.000000;0.000000;30.000000;5880.000000;MA_13_1054;MA_13_1055;*";
...
let codigoOriginal = itemDefault// nÃ£o mexer aqui <<
...
async function loadButton() {
    ...
    container.innerHTML = ""; // Limpa os botÃµes existentes

    let mapaDeIcones = {};
    let lines = [];

    // ?Ÿ”? Carregar Ã­cones primeiro
    ...
    // ?Ÿ”? Carregar os arquivos do array
    for (const file of buttonItems) {
        ...
        fileLines.shift(); // Remove o cabeÃ§alho
        lines.push(...fileLines);
    }

    // ?Ÿ”? Criar botÃµes para cada item
    lines.forEach((line) => {
    ...
    // ?Ÿ”? Aguarda a traduÃ§Ã£o dos nomes
...
async function carregarTraducoes() {
    const promessas = itemTranslations.map(arquivo =>
        fetch(arquivo + "?nocache=" + new Date().getTime())
            ...
            .catch(error => console.error(`?Œ Error in ${arquivo}:`, error))
    );
...
async function carregarCSV() {
    ...
    const promises = itemInformations.map(async (file) => {
        const response = await fetch(file + "?nocache=" + new Date().getTime());
        const blob = await response.blob(); // ObtÃ©m os dados como um Blob
    ...
    // Quando todos os arquivos forem carregados, vocÃª pode usar o array allItems
    itemArray = allItems;
}
...
async function carregarItemSetData() {
    ...
    const buffer = await response.arrayBuffer(); // LÃª como array buffer
...
async function carregarSetNameTranslations() {
    ...
    const text = await response.text();
    // Faz o parse do CSV usando vÃ­rgula como delimitador
    let parsed = Papa.parse(text, { header: true, skipEmptyLines: true, delimiter: "," });
    
    parsed.data.forEach(row => {
      if (row.t_key && row.t_korean) {
        // Remove os '^' e o sufixo _Name, converte a chave para minÃºsculo para padronizaÃ§Ã£o
        let key = row.t_key.replace(/\^/g, "").replace("_Name", "").trim().toLowerCase();

        // Armazena a traduÃ§Ã£o exatamente como estÃ¡ (mantendo a case original)
...
async function atualizarSetDiv(itemID) {
    // Carrega os dados, se ainda nÃ£o estiverem carregados
    ...
    if (!setRow) {
      // Item nÃ£o faz parte de nenhum set: oculta a div
      setDiv.style.display = "none";
      return;
    }
  
    // Caso o item faÃ§a parte do set, mostra a div
    ...
    // Conta quantos itens fazem parte do set (t_item1 a t_item14 que nÃ£o sÃ£o "*")
    let countItems = 0;
    for (let i = 1; i <= 14; i++) {
        ...
        if (campo && campo.trim() !== "*") {
            ...
            if (elem) {
                ...
                elem.style.display = "block";  // Certifica que estÃ¡ visÃ­vel
    ...
    // Verifica se seteffect1_count Ã© maior que 0
...
function atualizarItemacc() {
    ...
    if (partes.length == 66) {
        ...
        let descricaoItem = itemTraduzido.descricao || "DescriÃ§Ã£o nÃ£o encontrada"
        ...
        if (descricaoItem == "DescriÃ§Ã£o nÃ£o encontrada") {
        ...
        const coresRaridade = {
            ...
            4: "#ff00ff", // Roxo (LendÃ¡rio)
            6: "#ffaa55",  // laranja (Unique)
            7: "#ff1155"  // Vermelho (MÃ­tico)
        };
        ...
        switch (parseInt(rarity)) {
            ...
            case 7:
                ...
                break;
            default:
                rarityT = rarity; // Se nÃ£o for um dos casos, mantÃ©m o valor original
        }
        ...
        // Array para armazenar as partes do preÃ§o
        let precoFormatado = [];
        ...
        // Define os Ã­cones como imagens
...
function mudarItem(id) {
    ...
    if (!item) {
        console.error("Item nÃ£o encontrado:", id);
        return;
    }
    ...
    if (partes.length == 66) {
        ...
        console.log("NÃºmero de valores na linha:", contarValoresNaLinha(codigoBruto));
...
async function inicializarPagina() {
    ...
    if (!itemArray || Object.keys(mapaDeTraducoes).length === 0) {
        console.warn("?š ï¸? Dados ainda nÃ£o carregados. Tentando novamente em 100ms...")
